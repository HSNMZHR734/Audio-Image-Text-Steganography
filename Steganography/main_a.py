...
from audio_stg import AudioStg  # Assuming you have an audio_stg module for audio steganography
...
TEXT_PATH = ""
MEDIA_PATH = ""  # This will store the audio path
DEST_PATH = DIR_PATH
ERROR = ""
AUDIO_STG = AudioStg()  # Assuming you have a class AudioStg for audio steganography
...
def change_log(reset=False):
    ...
    if not reset:
        ...
        if LOGS["media_path"] == 0:
            text_log += "(?)-AUDIO FILE IS NOT SELECTED!\n\n"
        else:
...
def get_error(mode="embed"):
    ...
    if MEDIA_PATH == "":
        messagebox.showinfo("INFO", "AUDIO FILE IS NOT SELECTED!")
        return 1
...
def select_media_btn():
    ...
    if MEDIA_PATH.endswith(('.mp3', '.wav')):
        LOGS["media_path"] = 1
    else:
        MEDIA_PATH = ""
        LOGS["media_path"] = 0
...
def embed_btn():
    if get_error("embed") == 0:
        ...
        threading.Thread(target=AUDIO_STG.embed, args=(MEDIA_PATH, TEXT_PATH, DEST_PATH)).start()

def extract_btn():
    if get_error("extract") == 0:
        threading.Thread(target=AUDIO_STG.extract, args=(MEDIA_PATH, DEST_PATH)).start()
...
background_img = PhotoImage(file=compatible_path(RESOURCES + "background_a.png"))
